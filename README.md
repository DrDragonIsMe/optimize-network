...
- **网络拓扑自动检测**：从路由表提取网关 IP，在同网段探测不同 MAC 的宽带猫设备
...
```
笔记本 (192.168.0.x) → 路由器 (192.168.0.1) → 宽带猫 (192.168.1.1) → ISP
```

### 模式 B — 单网段（新式）

```
笔记本 (192.168.1.x) → 路由器 (192.168.1.1) → 宽带猫 (192.168.1.254) → ISP
```

检测逻辑：
1. **网关**：从系统路由表提取默认网关 IP（`netstat -rn` / `ip route`）
2. **宽带猫**：在网关同 `/24` 网段内依次探测 `.254`、`.1`、`.253`，通过 ARP 表确认 MAC 与网关不同
3. 如果自动检测失败，可通过环境变量手动指定 `ROUTER_IP` / `MODEM_IP`
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `ROUTER_IP` | （自动检测） | 路由器 ping 目标地址，留空则从路由表自动提取网关 |
| `MODEM_IP` | （自动检测） | 宽带猫 ping 目标地址，在网关同网段探测不同 MAC 的设备 |
...
脚本启动时自动检测网络拓扑，输出类似：
```
ℹ️  自动检测到默认网关: 192.168.1.1
ℹ️  自动检测到宽带猫: 192.168.1.254
ℹ️  网络拓扑: 笔记本 → 路由器(192.168.1.1) → 宽带猫(192.168.1.254) → ISP
```
